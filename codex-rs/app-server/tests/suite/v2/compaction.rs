...
async fn auto_compaction_local_emits_started_and_completed_items() -> Result<()> {
    ...
    assert!(
        completed_summary
            .as_deref()
            .is_some_and(|summary| summary.ends_with("LOCAL_SUMMARY"))
    );
...
async fn auto_compaction_remote_emits_started_and_completed_items() -> Result<()> {
    ...
    assert_eq!(started_id, completed_id);
    assert_eq!(started_summary, None);
    assert_eq!(completed_summary.as_deref(), Some("REMOTE_COMPACT_SUMMARY"));
...
async fn thread_compact_start_triggers_compaction_and_returns_empty_response() -> Result<()> {
    ...
    assert!(
        completed_summary
            .as_deref()
            .is_some_and(|summary| summary.ends_with("MANUAL_COMPACT_SUMMARY"))
    );
    assert_eq!(
        raw_completed,

...
pub(crate) async fn persist_tokens_async(
    ...
    let (previous_auth, auth) = persist_tokens_with_auth_history(
        codex_home,
        api_key,
        id_token,
        access_token,
    ...
    if previous_auth
        ...
        && let Err(err) = revoke_auth_tokens(previous_auth.as_ref(), auth_route_config).await
    {
...
async fn persist_tokens_with_auth_history(
    codex_home: &Path,
    api_key: Option<String>,
    id_token: String,
    access_token: String,
    ...
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    keyring_backend_kind: AuthKeyringBackendKind,
) -> io::Result<(Option<AuthDotJson>, AuthDotJson)> {

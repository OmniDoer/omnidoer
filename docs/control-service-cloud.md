...
freshness indicator and client-side stale-input refresh. The Control Service
also stores the delivered viewport dimensions and rejects pointer input outside
that pixel space before it reaches the browser worker. Mobile users can zoom
